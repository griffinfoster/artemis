...
parser.add_argument('-t', '--template', help='jinja2 LuMP template', default='LuMP_recorder.j2')
...
lumpDict = configData['LuMP']

lumpDict['generator_script'] = os.path.basename(__file__)

# check for start_date
if 'start_date' in lumpDict:
    lumpDict['opt_arg'] = '--start_date=%s'%lumpDict['start_date']

# override start_date
if not(args.start_date is None):
    if args.start_date.startswith('none'): lumpDict['opt_arg'] = ''
    else: lumpDict['opt_arg'] = '--start_date=%s'%args.start_date
